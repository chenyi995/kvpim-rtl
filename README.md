...
| 目录 | 内容 |
|---|---|
| `rtl/` | 正式组件 RTL（原 `rtl/0830-02/`，含 commit `a17696d` 的 SRAM 出口寄存器修复；BG buffer 为 flop 阵列，裁决 2026-09-02；文件集合 = `run_all.sh` 与 `testbench/run_tests.sh` 读入的集合）：bank GEMV、BG accumulator/buffer、logic-die 单元、softmax array、HBM controller + KV TLB |
...
## 最终结果（Genus/ASAP7，全部 run met；2026-09-02 BG buffer 改 flop 后重跑）
