...
| `rtl/accum_buffer_bg_asap7_macro.sv` | `rtl/accum_buffer_bg.sv` | BG accumulator buffer 的 SRAM 宏版（1 颗 512 B `srambank_64x4x16` 装 16 B / 128 B） | 裁决 2026-09-02 改 flop 阵列；宏版两档同面积，扩容零成本，与 bank 裁决不一致 |
...
| `syn/genus_0831_hier_reference/` | `syn/genus_0831_hier/` | 六个不进三类配置的参考 run（macro-buffer GEMV ×2、单体 dbuf ×2、单体 TLB、单体 recip）+ 宏版 BG buffer 两个旧 run（`accbuf_*_macro`）+ 驱动日志 `rerun*.log` | 见该目录 README；`gemv_attacc_p1501` 仍被 `collect.py` 读取算 13.18 mm²/die 锚点 |
