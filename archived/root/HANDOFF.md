...
| `kv_tlb_pkg.sv` / `kv_seg_tlb.sv` / `kv_ptw.sv` / `kv_scan_planner.sv` / `kv_tlb_top.sv` | New | KV **segment** TLB = drampim `CacheBlendTLB` in hardware | 32-entry range CAM + min-key iterate, PTW (directory + binary search, ATTACH), planner = `scan_runs`; see `KV_TLB.md` |
| `direct_addr_plan.sv` | New | AttAcc baseline address path | same plan port, one affine run, no table |
| `attacc_controller.sv` | New | instr FIFO + decode + cfg + run-based addr-gen + DRAM FSM | drives the plan port (`PIM_MAC_AB` → `OP_PLAN`, `PIM_ATTACH`) + `PIM_SET_META` |
...
**`kv_tlb_top`** (replaces the page-granular `tlb.sv`, 2026-08-28). Entries are
*segment descriptors* — drampim's `KVBlock` as seen by one consumer: logical
positions `[vpos_start, vpos_end)` of `(ctx, layer)` → `key_base` in a channel
pool (`value = key + 8 MiB`, 256-B vectors).  Lookup = 2-cycle range match +
address add; demand miss → `kv_ptw` (directory read + binary search over the
sorted segment array, one 32-B read per probe); `PIM_ATTACH` bulk-loads a
layer.  `kv_scan_planner` turns `[pos_lo, pos_hi)` into the same run list as
drampim's `scan_runs` (min-key iterate over resident entries, adjacency
merge); the controller consumes runs and decodes each 32-B column into
`{bank, row, col}` (ACT only on a row change).  Verified against vectors
generated by drampim's own `CacheBlendTLB` (`testbench/gen_kv_tlb_vectors.py`).
Measured: TLB hit 2 cyc, PTW demand walk ≈9.5 logic cycles (+ memory), plan
≈6 + 3/segment + 5/descriptor cycles — see `KV_TLB.md`.
...
- **AttAcc baseline = Fugue minus {TLB, rotate, diff_decoder}**, `kv_tlb_top`
  replaced by `direct_addr_plan` (one affine run, no table). Same controller in both (the `PIM_SET_META` decode
...
- **KV page table is read through a single-outstanding 32-B port** (`pt_mem_*`); DRAM timing of the walk is external (Ramulator's job).
...
5. **Scale + sensitivity** — sweep `SM_LANES` (→256), `NUM_GEMV`, `KV_TLB_ENTRIES` (`make kvtlb-sweep`),
