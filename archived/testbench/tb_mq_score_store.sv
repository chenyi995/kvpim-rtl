...
module tb_mq_score_store;
 logic clk=0,rst_n=0,mwr_valid,dwr_valid;logic[0:0]mwr_agent,dwr_agent,rd_agent;logic[2:0]mwr_idx,dwr_idx,rd_idx;logic[3:0]mwr_drop,dwr_lanes;logic[3:0][31:0]mwr_data,dwr_data,rd_data;always #5 clk=~clk;mq_score_store #(.AGENTS(2),.LANES(4),.NWORDS(2))dut(.*);
