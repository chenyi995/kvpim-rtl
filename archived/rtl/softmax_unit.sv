...
module softmax_unit import fugue_pkg::*; #(
    ...
    always_comb begin
        ...
        for (int l = 1; l < LANES; l++)
            mx[l] = fp32_gt(score[l], mx[l-1]) ? score[l] : mx[l-1];
    end
    wire [31:0] gmax_c = mx[LANES-1];

    // ---- FSM ----
    typedef enum logic [3:0] {
        IDLE, MAX, SUB_ISSUE, SUB_WAIT, EXP_WAIT,
    ...
    // ---- LANES FP32 subtractors (score - gmax) ----
    genvar l;
    generate
        for (l = 0; l < LANES; l++) begin : g_sub
            logic v;
            fp32_add u_sub (.clk(clk),.rst_n(rst_n),.in_valid(sub_iv),
                            .a(score[l]),.b(neg_gmax),.y(sub_y[l]),.out_valid(v));
            if (l == 0) assign sub_v = v;
        end
    endgenerate

    // ---- LANES FP32 exp, fed directly from the subtractors when they fire ----
    generate
        for (l = 0; l < LANES; l++) begin : g_exp
            logic v;
            fp32_exp u_exp (.clk(clk),.rst_n(rst_n),.in_valid(sub_v),
                            .x(sub_y[l]),.y(exp_y[l]),.out_valid(v));
            if (l == 0) assign exp_v = v;
        end
    endgenerate
    ...
    // ---- LANES FP32 normalize multiplies (exp * 1/sum) ----
    generate
        for (l = 0; l < LANES; l++) begin : g_mul
            logic v;
            fp32_mul u_mul (.clk(clk),.rst_n(rst_n),.in_valid(norm_iv),
                            .a(e_reg[l]),.b(recipv),.y(mul_y[l]),.out_valid(v));
            if (l == 0) assign mul_v = v;
        end
    endgenerate
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            unique case (state)
                IDLE:       if (start) begin score <= in_data; state <= MAX; end
                MAX:        begin gmax <= gmax_c; state <= SUB_ISSUE; end
