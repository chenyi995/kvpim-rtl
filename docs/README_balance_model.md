...
I(n, f) = max(  6,                                   # DRAM 协议地板
                ...
                6 · ( E_col + n·E_op·ê(f) ) / E₆ )   # 能量项：花超预算按比例拉长
```
...
推论（ê 平坦近似下的解析交点）：compute 项 = 能量项给出
f\* ≈ n / (0.769 · 6 · (E_col + n·E_op)/E₆)。代入数值：

| n | 能量项 I_pw（ê=1） | 平坦近似 f\* |
|---|---|---|
| 1 | 6.0 tCK（原版，永在地板） | —（0.22 GHz 即够，DRAM 限） |
| 4 | 6.6 tCK | ≈ 1.05 GHz |
| 8 | 7.5 tCK | ≈ 1.39 GHz |
| 16 | 9.2 tCK | ≈ 2.25 GHz |
| 32 | 12.7 tCK | ≈ 3.28 GHz |

实际 f\* 比平坦近似**低**（高频 ê 上翘），以扫描数据为准。
