...
**而且 1.3 GHz 不需要任何流水加深**（直接验证，
`raw_data/mq16x2mp1_769ps/`）：把乘法器/加法器都保持 AttAcc 原版的
**单拍**实现（`mq_pe_16x2_d32_mp1`，MPIPE=APIPE=1，只有加法树固有的
5 级层间寄存器），769 ps 综合 **met、slack 0**——14,120 µm²、
115.7 mW，且比 2+2 流水版（13,961 µm²、130.1 mW）**能量低 11%**
（少了流水寄存器的时钟功耗）。深流水（2+2、4+4）只是扫描中探索
3 GHz 以上区域的工具，平衡点本身用不到。
