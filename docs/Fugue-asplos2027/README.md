...
f ≥ 8/(8×0.769) = **1.31 GHz**。**平衡点 ≈1.5 GHz（实测频点；理论门槛
1.31 GHz），I = 8 tCK**。之后间隔被功耗预算精确钉死在 8：1.5→3.0 GHz
收益为 **0**；6 tCK 地板在能量口径下不可达（要到 ~3.3 GHz、ê≤0.71
才降到 7 tCK——为 1 tCK 付 2.2× 频率，不值）。

| 设计点 | PE 时钟 | 间隔 I | 每 ACT 计算量 | 相对原版 |
|---|---|---|---|---|
| AttAcc 原版（n=1） | 0.67 GHz | 6 tCK（地板） | 64 MAC | 1× |
| 原版 PE 直接跑 MQ n=8 | 0.67 GHz | 16 tCK | 512 MAC | 计算受限 |
| **Fugue 平衡点（n=8）** | **≈1.5 GHz（≥1.31）** | **8 tCK** | **512 MAC** | **间隔 -50%，吞吐 2.0×** |
...
平衡值 **8 tCK**（每 8 tCK 一次列读、8 次 16-lane MAC）；再提频被
DRAM 功耗预算精确钳在 8 tCK、收益为零，属于面积/时序的纯浪费。
频率代价温和：13.5k → 14.1k µm²/PE（+3.9%，0.67→1.5 GHz，ASAP7）；
...
5. 能量常数与仿真器（`attacc_drampim_822`）完全同源，间隔取整口径也
   ...
   n_cap=8 → **1.5 GHz / interval 8 tCK**（替代现行 1.733 GHz / 6 tCK
   的"恰好塞满"配平，后者在能量口径下不可达）；n_cap=16 →
   2.6 GHz / 9 tCK。
