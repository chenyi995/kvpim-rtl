# kvpim-rtl 导航（2026-09-02 版，分支 `chenyi-genus-0902`）

本分支已整理：只保留进论文的正式代码与结果，其余全部在 `archived/`
（对照表见 `archived/README.md`）。两条线的关系：

| 线 | 分支 | 内容 |
|---|---|---|
| **组件级 overhead 线（本分支）** | `chenyi-genus-0902`（源自 `xinyao_0831`） | `rtl/` 组件集 + Genus 层次化综合矩阵 + 层级 roll-up |
| 平衡点/论文线 | `xinyao_0825` | in-bank PE 频率-能量平衡点（1.3 GHz / 8 tCK）、论文证据包 `docs/Fugue-asplos2027/`、`docs/README_balance_model.md`、`CLAUDE.md` 操作守则 |

## RTL

- **`rtl/`（正式）**：按 `docs/Hardware Overhead.md` 规格重写的组件集
  （bank GEMV / BG accumulator / logic-die 单元 / HBM controller + TLB）。
  **含 SRAM 出口流水寄存器修复**（commit `a17696d`：
  `dbuf_16x256_asap7.sv`、`dbuf_16x256.sv`、`gemv_unit.sv`，读延迟 2 拍，
  裁决记录见 `syn/genus_0831_hier/README.md`）。
- `archived/rtl/`：N28 logic-die 时代的实现，组件线不再使用。

## 综合结果：信谁

| 目录 | 工具 | 状态 |
|---|---|---|
| **`syn/genus_0831_hier/`** | Genus/ASAP7，层次化 leaf-as-macro | **权威**。修复后 RTL、诚实的 SRAM arc、retiming 开。方法学与两项上游修复见其 `README.md`，结果表 `SUMMARY.md` |
| `archived/syn/dc_0830-02/` + `archived/docs/0830-02/Summary.md` | DC/ASAP7，组件级 | **部分过时**，见 Summary 末尾勘误：①四个 timing 失败其实 retiming 能收；②所有经 SRAM 的 "met" 是假阳性（DC 丢弃了 lib 的非法 clk→dataout arc）；③GEMV 各行对应修复前 RTL |
| `archived/syn/xinyao_0828/` | DC，早期 SRAM 基线 | 自述 rough/debug |

## docs/ 里各文件

- **`0831-genus-hier/`** — 0831 Genus 战役的记录页（结果/过程/全部改动/
  原始数据路径）+ **`DATA_README.md` 与两个 CSV（写论文画图直接用）**
- `Hardware Overhead.md` — 组件线的任务规格（xinyao 手写，四层级定义与频率契约）
- `ASAP7_SRAM_AREA_COMPARISON.md` — SRAM macro 面积比较
- roll-up 公式与计数现以 `syn/genus_0831_hier/collect.py` 为准
  （N_gemv=1024、N_bg=256、N_ch=16、bank/BG ×10 DRAM 等效）；
  原推导文档在 `archived/docs/0830-02/Hardware_Overhead_Breakdown.md`

## 三条关键结论（2026-08-31）

1. **Retiming 是必需项**：单拍 FP16 乘/加在 1.3 GHz 靠 retiming 收敛。
2. **上游 SRAM liberty 有两处坑**：内部数值尺度混写（ns 声明下混有
   ps/fF 量级表）与 `area:0`；`genus_0831_hier/convert_sram_libs.py`
   修复并从 LEF 注入面积。DC 用原始 lib 的 SRAM 路径时序等于未约束。
3. **latch 型 srambank 低相位出数 → 出口必须打一拍**：修复后 GEMV 在
   666 MHz / 1.3 GHz 双双 met。`softmax_buffer_sram` 经整合 array 验证
   无需此修复（FSM WAIT 吸收半周期）。

## 四项裁决（chenyi9 2026-09-01 / 09-02，已体现在 SUMMARY/CSV）

- **N_gemv=1024**（AttAcc Fig.9(b) 两 bank 共享 2 GEMV）。
- bank/BG **×10** DRAM 工艺等效。
- **bank buffer 用 flop 阵列**（512 B/份时 flop 3.8k µm² < macro 最优
  6.0k；盈亏平衡 ~1 KiB/份）。
- **BG buffer 各取最优实现**（2026-09-02）：原宏版把 16 B / 128 B 装进同一颗
  512 B 宏，两档同面积，扩容零成本。比较后 AttAcc 16 B 用 flop（62 µm²，
  宏 250），Fugue 128 B 用宏（251 µm²，flop 448）。
  stack 合计 Fugue vs AttAcc：ASAP7 **+26.44%**、DRAM 等效 **+13.49%**。

## 待办

- [ ] DC 矩阵按勘误复核（开 retime、换 `libs_ps/`、用修复后 RTL）——若不打算再用 DC 数字可关闭
