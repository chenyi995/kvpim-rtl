...
// 6-cycle latency (registered output). out_valid follows in_valid through the SAME
// six register stages as the data. Subnormals are flushed to zero. Round-to-nearest-
...
module fp32_recip (
    ...
    // ---------------------------------------------------------------------------
    // Stage 1b: four 12x16 partial products for the first Newton multiply.
    // ---------------------------------------------------------------------------
    // P is Q(23+30)=Q53 in 56 bits (value m*r0, in ~[0.5,2)).
    logic [27:0] p0_ll_s1b, p0_lh_s1b, p0_hl_s1b, p0_hh_s1b;
    logic [31:0] seed_s1b;
    logic [7:0]  ea_s1b;
    logic [1:0]  cls_s1b;
    logic        v_s1b;
    logic [55:0] prod_P;
    assign prod_P = {28'd0, p0_ll_s1b} +
                    {16'd0, p0_lh_s1b, 12'd0} +
                    {16'd0, p0_hl_s1b, 12'd0} +
                    {p0_hh_s1b, 28'd0};
    ...
    // ---------------------------------------------------------------------------
    // Stage 2 combinational: form four 16x16 partial products for the second
    // Newton multiply.  A monolithic 32x32 multiply did not close at 666 MHz.
    // ---------------------------------------------------------------------------
    // r0 (Q2.30) * two (Q2.30) = Q60 in 64 bits.  The partial products are
    // registered, then combined in a separate stage.
    logic [31:0] pp_ll_s3, pp_lh_s3, pp_hl_s3, pp_hh_s3;
    logic [7:0]  ea_s3;
    logic [1:0]  cls_s3;
    logic        v_s3;

    // ---------------------------------------------------------------------------
    // Stage 3 combinational: combine the registered partial products.
    // ---------------------------------------------------------------------------
    logic [63:0] prod_R_s3;
    logic [32:0] pp_cross_s3;
    assign pp_cross_s3 = {1'b0, pp_lh_s3} + {1'b0, pp_hl_s3};
    assign prod_R_s3 = {pp_hh_s3, 32'd0} + {15'd0, pp_cross_s3, 16'd0} + pp_ll_s3;
    ...
    // ---------------------------------------------------------------------------
    // Stage 4 combinational: normalize, round (RNE), exponent, pack
    // ---------------------------------------------------------------------------
    ...
    always_comb begin
        if      (r1_s4[30]) begin lead_p = 2'd2; p_val = 5'd30; end
        else if (r1_s4[29]) begin lead_p = 2'd1; p_val = 5'd29; end
    ...
    assign exp_base = $signed({4'd0, p_val}) + 11'sd224 - $signed({3'd0, ea_s4});
    ...
    // ---------------------------------------------------------------------------
    // Pipeline registers (5 stages: s1, s2, partial products, combine, output).
    // ---------------------------------------------------------------------------
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            v_s1      <= 1'b0;
            // Stage 1b
            p0_ll_s1b <= 28'd0; p0_lh_s1b <= 28'd0;
            p0_hl_s1b <= 28'd0; p0_hh_s1b <= 28'd0;
            seed_s1b  <= 32'd0;
            ea_s1b    <= 8'd0;
            cls_s1b   <= 2'b00;
            v_s1b     <= 1'b0;
            ...
            cls_s2    <= 2'b00;
            v_s2      <= 1'b0;
            // Stage 3: four parallel 16x16 partial products
            pp_ll_s3  <= 32'd0; pp_lh_s3 <= 32'd0;
            pp_hl_s3  <= 32'd0; pp_hh_s3 <= 32'd0;
            ea_s3     <= 8'd0;
            cls_s3    <= 2'b00;
            v_s3      <= 1'b0;
            // Stage 4
            ...
            v_s1      <= in_valid;
            // Stage 1b <- Stage 1
            p0_ll_s1b <= mant_M_s1[11:0]  * seed_s1[15:0];
            p0_lh_s1b <= mant_M_s1[11:0]  * seed_s1[31:16];
            p0_hl_s1b <= mant_M_s1[23:12] * seed_s1[15:0];
            p0_hh_s1b <= mant_M_s1[23:12] * seed_s1[31:16];
            seed_s1b  <= seed_s1;
            ea_s1b    <= ea_s1;
            cls_s1b   <= cls_s1;
            v_s1b     <= v_s1;
            ...
            v_s2      <= v_s1b;
            // Stage 3 <- Stage 2
            pp_ll_s3  <= seed_s2[15:0]  * two_s2[15:0];
            pp_lh_s3  <= seed_s2[15:0]  * two_s2[31:16];
            pp_hl_s3  <= seed_s2[31:16] * two_s2[15:0];
            pp_hh_s3  <= seed_s2[31:16] * two_s2[31:16];
            ea_s3     <= ea_s2;
            cls_s3    <= cls_s2;
            v_s3      <= v_s2;
            // Stage 4 <- Stage 3
            r1_s4     <= prod_R_s3[61:30];
            ea_s4     <= ea_s3;
            cls_s4    <= cls_s3;
            v_s4      <= v_s3;
            // Output <- Stage 4
