...
package fugue_pkg;
  ...
  // ---- Structural counts -------------------------------------------------
  // The current simulation models one HBM channel.  Keep its four GEMV PEs,
  // but instantiate a single softmax pipeline/SRAM buffer and merge decoder.
  localparam integer NUM_GEMV = 4;
  // A softmax unit consumes one 16-FP32 SIMD chunk per accepted cycle.
  localparam integer SM_LANES = 16;
  localparam integer SM_UNITS = 1;
  localparam integer SM_TOTAL_LANES = SM_UNITS * SM_LANES;
  // MAX_L=4096 requires 256 sixteen-score chunks and corresponding diff masks.
  localparam integer SM_WORDS = 256;
