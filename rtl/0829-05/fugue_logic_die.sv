...
module fugue_logic_die import fugue_pkg::*; import kv_tlb_pkg::*; (
    input  logic                       clk,
    // Dedicated MQ PE clock.  All other logic-die blocks remain on clk.
    input  logic                       pe_clk,
    input  logic                       rst_n,
    ...
    input  logic [SM_UNITS-1:0][SM_LANES-1:0][31:0]         diff_stream,
    ...
    output logic [SM_UNITS-1:0][SM_LANES-1:0][31:0]         ctx_to_master,
    output logic [SM_UNITS-1:0][SM_LANES-1:0][31:0]         ctx_to_diff,
    ...
    // ---------------- NUM_GEMV MQ GEMV PEs + accumulator ----------------
    // Area/timing model: the MQ PE is a separate 1.3004-GHz clock domain.
    // The present controller keeps its existing low-frequency command protocol;
    // result-valid is retained as a structural accounting boundary, not a
    // cycle-accurate CDC implementation.
    logic [NUM_GEMV-1:0][15:0] gemv_res;
    logic [NUM_GEMV-1:0]       gemv_rv;
    logic [NUM_GEMV-1:0][255:0] mq_drain_data;
    logic [NUM_GEMV-1:0]        mq_busy;

    genvar g;
    generate
        for (g = 0; g < NUM_GEMV; g++) begin : g_gemv
            mq_pe_16x2_d32 u_mq_pe (
                .clk(pe_clk), .rst_n(rst_n),
                .vec_wr_en(vec_wr_en), .vec_wr_addr({1'b0, vec_wr_addr}),
                .vec_wr_data(vec_wr_data), .vec_swap(vec_swap),
                .col_wr_en(mat_wr_en), .col_data(mat_wr_data),
                .col_is_second(gemv_row_addr[0]), .ctx_word_sel(gemv_vec_addr[0]),
                .start(gemv_start), .n_active(5'd16), .op_is_score(op_is_score),
                .slot_base({1'b0, gemv_vec_addr}), .token_lane(gemv_row_addr),
                .stage_swap(gemv_accum_clr), .drain_en(gemv_accum_en),
                .drain_sel_ctx(!op_is_score), .drain_slot(gemv_row_addr),
                .drain_word(gemv_vec_addr[0]), .drain_data(mq_drain_data[g]),
                .busy(mq_busy[g])
            );
            assign gemv_res[g] = mq_drain_data[g][15:0];
            assign gemv_rv[g]  = gemv_start;
        end
    endgenerate
    ...
    logic [SM_UNITS-1:0][SM_LANES-1:0][31:0] corrected_score, sm_out;
    logic [SM_UNITS-1:0][SM_LANES-1:0] corrected_mask;
    logic [SM_UNITS-1:0] corrected_valid, sm_busy, sm_done;
    // The single simulated channel owns one buffer, decoder state, and
    // probability stream.
    generate
        for (g = 0; g < SM_UNITS; g++) begin : g_softmax
            diff_decoder u_diff (
                .clk(clk), .rst_n(rst_n),
                .meta_wr_en(meta_wr_en), .meta_wr_idx(meta_wr_idx), .meta_wr_mask(meta_wr_mask),
                .fwd_valid(sm_in_valid[g]), .fwd_word_idx(sm_word_idx),
                .master_score(sm_scores[g]), .diff_stream(diff_stream[g]),
                .corrected_score(corrected_score[g]), .corrected_mask(corrected_mask[g]),
                .corrected_valid(corrected_valid[g]),
                .rev_valid(sm_valid[g]), .rev_word_idx(sm_word_idx), .prob(sm_out[g]),
                .to_master(ctx_to_master[g]), .to_diff(ctx_to_diff[g]), .rev_valid_o()
            );
            buffered_softmax_unit #(.MAX_L(4096)) u_sfm (
                .clk(clk), .rst_n(rst_n),
                .in_valid(corrected_valid[g]), .in_ready(sm_in_ready[g]),
                .in_data(corrected_score[g]), .seq_len(cfg_seqlen),
                .out_valid(sm_valid[g]), .out_ready(sm_out_ready[g]), .out_data(sm_out[g]),
                .out_keep(sm_keep[g]), .out_last(sm_last[g]), .done(sm_done[g])
            );
    ...
    wire _unused = rotate_start | (|rotate_pos) | (|cfg_nhead) | (|cfg_dhead)
                 | (|cfg_seqlen) | (|sm_busy) | (|sm_done) | sfm_start | addr_fault | (|kv_run_value_base) | (|corrected_valid)
                 | (|corrected_mask);
