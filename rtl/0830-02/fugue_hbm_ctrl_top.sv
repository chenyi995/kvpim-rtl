// fugue_hbm_ctrl_top.sv — synthesis top for the Fugue HBM controller level:
// the same controller core + DMA engine as AttAcc, plus the KV segment TLB
// (kv_tlb_top = drampim's CacheBlendTLB in hardware: segment CAM + PTW +
// scan planner) serving the address-plan port for master/diff-block address
// management.  The page-table read port is exposed (pt_mem_*).
// One instance per HBM.  Target 666 MHz (docs/Hardware Overhead.md).
module fugue_hbm_ctrl_top import fugue_pkg::*; import kv_tlb_pkg::*; (
    input  logic                 clk,
    input  logic                 rst_n,
    ...
    output logic [31:0]          cfg_kvbase,     // page-table directory base
    ...
    // ---- page-table memory port (single outstanding 32-B read) ----
    ...
    // ---- DMA: descriptor + read/write masters ----
    ...
    assign run_key_base = run.key_base;
    assign run_count    = run.count;
    ...
    attacc_controller u_ctrl (
        ...
        .run_valid, .run_ready,
        .run_key_base, .run_count, .addr_fault,
    ...
    kv_tlb_top u_tlb (
        .clk, .rst_n,
        .cfg_dir_base ({cfg_kvbase_i[KV_ADDR_W-6:0], 5'b0}),  // CFG_KVBASE is in 32-B units
        .cmd_valid    (plan_cmd_valid),
        .cmd_ready    (plan_cmd_ready),
        .cmd_op       (plan_cmd_op),
        .cmd_ctx      (plan_cmd_ctx),
        .cmd_layer    (plan_cmd_layer),
        .cmd_pos_lo   (plan_cmd_pos_lo),
        .cmd_pos_hi   (plan_cmd_pos_hi),
        .cmd_pools    (plan_cmd_pools),
        .cmd_done     (plan_cmd_done),
        .cmd_fault    (plan_cmd_fault),
        .run_valid    (run_valid),
        .run_ready    (run_ready),
        .run          (run),
        .run_value_base(run_value_base),
        .mem_req_valid (pt_mem_req_valid),
        .mem_req_ready (pt_mem_req_ready),
        .mem_req_addr  (pt_mem_req_addr),
        .mem_resp_valid(pt_mem_resp_valid),
        .mem_resp_data (pt_mem_resp_data)
    );
    ...
    logic _unused;
    assign _unused = ^{run_value_base, run.kind, run.ch_base, run.ch_count};
endmodule
